...
void enqueue(queue_t *queue, pcb_t *process)
{
    ...
    if (is_empty(queue)) {
        ...
        if (scheduler_algorithm == PA) {
            double func_priority = priority_with_age(get_current_time(), process);
            double head_func_priority = priority_with_age(get_current_time(), queue->head);
            if (func_priority > head_func_priority) {
                process->next = queue->head;
                queue->head = process;
            } else {
                pcb_t * curr = queue->head;
                while (curr->next && priority_with_age(get_current_time(), curr->next) > func_priority) 
                    curr = curr->next;
...
pcb_t *dequeue(queue_t *queue)
{
    pcb_t *process;
    pthread_mutex_lock(&queue_mutex);
    if (is_empty(queue) && !pthread_mutex_unlock(&queue_mutex)) return NULL;
...
extern void wake_up(pcb_t *process)
{
    ...
    if (scheduler_algorithm == PA) {
       ...
       for (unsigned int i = 0; i < cpu_count; i++) {
            ...
            double qfunc_priority = priority_with_age(get_current_time(), current[i]);
            if (qfunc_priority < min)  {
                min = qfunc_priority;
                pid = i;
            }  
       }
       ...
       pthread_mutex_unlock(&current_mutex);
       if (min < func_priority) force_preempt(pid);
    }
}
