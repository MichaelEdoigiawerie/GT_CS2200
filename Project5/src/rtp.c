...
packet_t *packetize(char *buffer, int length, int *count) {
    ...
    int temp = MAX_PAYLOAD_LENGTH;

    if (length % MAX_PAYLOAD_LENGTH) {
        temp = length % MAX_PAYLOAD_LENGTH;
        numPackets++;
    }
    packets = (packet_t *) malloc(sizeof(packet_t) * (size_t) numPackets);
    ...
    packets[numPackets - 1].payload_length = temp;
    for (int i = 0; i < temp; i++)
        packets[numPackets - 1].payload[i] = buffer[(numPackets - 1) * MAX_PAYLOAD_LENGTH + i];
    packets[numPackets - 1].checksum = checksum(packets[numPackets - 1].payload, temp);
...
int checksum(char *buffer, int length) {
    ...
    for (int i = 0; i < length; i++) {
        if (i && i % 2 == 0) conv = conv == 3.0 ? (double) (1 / 3.0) : 3.0;
        result += (int) (buffer[i] * conv);
    }
    return result;
}
            ...
            */
            if (packet.type == LAST_DATA || packet.type == DATA) {
                ...
                if (checksum(packet.payload, packet.payload_length) == packet.checksum) {
                    temp->type = ACK;
                    unsigned long size = (unsigned long) (buffer_length + packet.payload_length);
                    buffer = buffer ? realloc(buffer, size) : (char *) malloc((unsigned long) packet.payload_length);
